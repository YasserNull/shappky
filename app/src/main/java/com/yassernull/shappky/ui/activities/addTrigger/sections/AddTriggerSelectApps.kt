...
import com.yassernull.shappky.ui.activities.addTrigger.AddTriggerSelectAppsDialogs
import com.yassernull.shappky.utils.loadAllApps

@Composable
fun SelectAppsSection(
  ...
  Row(
    ...
    Column(modifier = Modifier.weight(1f)) {
      ...
      Text(
        text = if (excludedApps.isEmpty()) "0" else "${excludedApps.size}",
        fontSize = 14.sp,
  ...
  Row(
    ...
    Column(modifier = Modifier.weight(1f)) {
      ...
      Text(
        text = if (manuallySelectedApps.isEmpty()) "0" else "${manuallySelectedApps.size}",
        fontSize = 14.sp,
        color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.7f),
      )
    }
