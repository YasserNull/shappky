...
suspend fun Context.collectAutoBackgroundPackages(shellManager: ShellManager): Set<String> = withContext(Dispatchers.IO) {
  try {
    val psOutput = shellManager.runShellCommandAndGetFullOutput(
      "${ShellManager.TOYBOX_PATH} ps -A -o pid,name | grep '\\.' | grep -v '[-@]'",
    ) ?: ""
    ...
    runningPackages - (foregroundPackage?.let { setOf(it) } ?: emptySet()) - packageName
  } catch (_: Exception) {
    emptySet()
  }
}
