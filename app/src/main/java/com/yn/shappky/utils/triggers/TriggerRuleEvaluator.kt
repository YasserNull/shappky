  ...
  fun evaluateInactivityRules(
    ...
    for (trigger in triggers) {
      ...
      val candidatePackages = runningPackages.filter { pkg ->
        if (pkg == "com.yn.shappky" || protectedApps.contains(pkg) || excludedApps.contains(pkg)) return@filter false
    ...
    val candidatePackagesForServiceRules = runningPackages.filter { it != "com.yn.shappky" }
